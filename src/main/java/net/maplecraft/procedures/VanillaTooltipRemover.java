...
import net.maplecraft.utils.BaseEquipItem;
...
public class VanillaTooltipRemover {
    ...
    public void onRenderTooltip(RenderTooltipEvent.Pre event) {
        ...
        if (itemStack.getItem() instanceof BaseEquipItem item && item.tooltip.size() != event.getComponents().size()) {
            event.setCanceled(true);
            // fire a new event with custom tooltip only
            assert Minecraft.getInstance().screen != null;
            Minecraft.getInstance().screen.renderComponentTooltip(event.getPoseStack(), item.tooltip, event.getX(), event.getY());
        }
    }
}
